...
void CChunkAllocator::init_pool_data(int n_chunks, int chunk_size)
{
    ...
    {
        offset = i * get_chunk_size(n_chunks, chunk_size);
...
int CChunkAllocator::verify_pool_data(int n_chunks, int chunk_size)
{
    ...
    if((chunk_->chunk_size_ != chunk_size) || 
        (chunk_->chunk_total_ != n_chunks))
    {
        ERROR_RETURN_2(CHUNK_ALLOCATOR_ERROR_DATA_VERIFY_FAIL, "data verify fail[total:%d, size:%d]", 
                            chunk_->chunk_total_, chunk_->chunk_size_);
    }
    ...
    {
        ERROR_RETURN_2(CHUNK_ALLOCATOR_ERROR_DATA_VERIFY_FAIL, "data verify fail[free_total:%d, free_list:%d]", 
                            chunk_->free_total_, free_total);
...
{
    int ret = 0;
    size_t pool_size = get_pool_size(n_chunks, chunk_size);
