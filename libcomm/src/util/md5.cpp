...
MD5Sum_test::MD5Sum_test(const char* sum)
{
	init();
	m_committed = true;
...
bool MD5Sum_test::isCommitted() const
{
	//return (m_committed && !m_bpos);
	return m_committed;
}

void MD5Sum_test::put(const char* buf, unsigned int size)
{
	while(size--) {
		m_buf[m_bpos++] = *(buf++);
		if(m_bpos >= 64)
...
MD5Sum_test::operator const unsigned char*()
{
	commit();
	return m_md5;
}

string MD5Sum_test::toString()
{
	commit();
	return toString(m_md5);
}

string MD5Sum_test::toTempString()
{
	char buf[512];
	//save state
	memcpy(buf, this, sizeof(MD5Sum_test));
	string result = toString();
	//restore state
	memcpy(this, buf, sizeof(MD5Sum_test));
	return result;
}

string MD5Sum_test::toString(const unsigned char* md5)
{
	char buf[36];
...
bool MD5Sum_test::operator==(const MD5Sum_test& sum)
{
	commit();
...
string getStringMD5Value(const char *str)
{
	MD5Sum_test md5;
...
{
	MD5Sum_test md5;
	md5.put((const char *)input,len);
	memcpy(output, (const unsigned char *)md5, 16);
}
...
int main(int argc, char **argv)
{
	...
	string md5sum = getStringMD5Value(argv[1]);
	printf("Md5sum=%s\n", md5sum.c_str());
	char r[16];
	Md5Sum(r, argv[1], strlen(argv[1]));
	printf("Md5sum=");
	for(int i = 0; i < 16; i++)
	{
